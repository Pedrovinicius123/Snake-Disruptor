...
double check_obstacles(char current_direction, int snake_size, int**next_snake, int*current_pos){
  double count = 0;
  for (int i = 1; i < snake_size; i++){
    if (current_direction == 'S'){
      if (current_pos[0] == next_snake[i][0] && current_pos[1] < next_snake[i][1]){
      ...
      if (current_pos[1] == next_snake[i][1] && current_pos[0] > next_snake[i][0]){
        count += pow(calculate_distance(current_pos, next_snake[i]), 2) * pow(calculate_distance_from_border('W', current_pos), -1);
      }
    }if (current_direction == 'S'){
      if (current_pos[0] == next_snake[i][1] && current_pos[0] < next_snake[i][0]){
...
bool check_collision(int snake_size, int* current_pos, int** next_snake){
  int next[2];
  if (current_direction == 'W') {
  ...
  if (!check_position_in(next, next_snake, snake_size) && !check_position_out_of_board(next, 380)) {
    return true;
  } return false;

}

char compare_obstacles(char current_direction, char* others, int snake_size, int**next_snake, int*current_pos){
  double count = check_obstacles(current_direction, snake_size, next_snake, current_pos);
  char result = current_direction;
  for (int i = 0; i < 3; i++){
    double inner = check_obstacles(others[i], snake_size, next_snake, current_pos);
    if (inner < count){
      count = inner;
      result = others[i];
...
char set_direction(char current_direction, char anterior_direction, int snake_size, int chunk, bool away, int* current_pos, int* fruit_pos, int** snake, int**next_snake, char* chosed) {
    ...
    printf("CHOSED %c\n", chosed_copy[0]);
    if (chosed_lenght <= 1){
      return chosed_copy[0];

    }

    char choice = compare_obstacles(chosed_copy[0], chosed_copy, snake_size, next_snake, current_pos);
    return set_direction(choice, chosed_copy[0], snake_size, chunk, false, current_pos, fruit_pos, snake, next_snake, chosed_copy);
